...
def project_gray(i_y):
    # Computes the mean along the second (zero-indexed) axis, that is the
    # mean of all channels for each pixel.
    i_y_mean = np.mean(i_y, axis=2)
    result = np.zeros(i_y.shape)
    for i in range(3):
        result[:,:,i] = i_y_mean
        
    # Three channels with the mean value in each channel.
    return result

def project_chromaticity(i_y):
    # Per channel (color), compute the derivation from the mean as
    # chomaticity.
...
class GrayscaleRetinexEstimator:
    ...
    @staticmethod
    def param_choices():
        # Returns a vector of evenly spaced values in logspace (15 values
        # between -3 and 1 with base 10).
