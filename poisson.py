...
def get_gradients(I):
    ...
    of an image (per channel, that is returns 3 channels output for a RGB image) """
...
def get_numbers(mask):
    """ Returns an array where in each cell, the number of pixels (i,j) with
    mask(i,j) == 1 already visited before is stored. """
    M, N = mask.shape
